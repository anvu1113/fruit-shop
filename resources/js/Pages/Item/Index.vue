<template>
  <form @submit.prevent="filter">  
    <div class="mb-8 mt-4 flex justify-end gap-2">       
      ...
      <Link :href="route('item.create')" class="btn-primary flex" as="button">
  ...
  <div class="relative overflow-x-auto">
    <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
      ...
      <tbody>
        <template v-if="items.data.length">
          <tr v-for="(item, key) in items.data" :key="item.id" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
            ...
            <td scope="row " class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white flex">
              ...
              <a class="cursor-pointer" type="button" @click="deleteItem(item.id)">
        ...
        <template v-else>
          <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
            <td scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white text-center" colspan="8">
              No data
            </td>            
          </tr>      
        </template>         
    ...
    <div v-if="items.data.length" class="w-full flex justify-center mt-4 mb-4">
...
const pageCurrent = ref(props.items.current_page)
...
const deleteItem = (id) => {
  if (confirm('Are you sure you want to delete this item?') && id > 0) {
    useForm({}).delete(route('item.destroy', id))
  }
}
